...
export function normalizePicks(picks:SleeperPick[]):NormalizedDraftPick[] { return picks.map(p=>({externalId:`${p.pick_no}:${p.player_id}`,pick:p.pick_no,round:p.round,slot:p.draft_slot,playerExternalId:p.player_id,playerName:[p.metadata?.first_name,p.metadata?.last_name].filter(Boolean).join(" ")||undefined,teamExternalId:p.roster_id?String(p.roster_id):p.picked_by,isKeeper:Boolean(p.is_keeper||p.metadata?.is_keeper==="true"),selectedAt:p.metadata?.picked_at?new Date(Number(p.metadata.picked_at)):undefined,metadata:p.metadata})); }
...
export async function hydrateSleeperLeague(raw:SleeperLeague,currentUserId?:string):Promise<NormalizedLeagueProfile>{
  ...
  const managers=rawUsers.map(u=>normalizeUser(u,currentUserId)); const teams=normalizeTeams(rosters,rawUsers); const keepers=(currentDraft?.picks??[]).filter(p=>p.isKeeper).map(p=>({playerExternalId:p.playerExternalId,playerName:p.playerName,teamExternalId:p.teamExternalId??String(p.slot),round:p.round,sourcePickId:p.externalId,originalPickMetadata:p.metadata}));
...
export async function sleeperLeague(id:string,currentUserId?:string){return hydrateSleeperLeague(await get<SleeperLeague>(`/league/${id}`),currentUserId)}
export const sleeperProvider:DraftPlatformProvider={name:"Sleeper",user:async username=>normalizeUser(await get<SleeperUser>(`/user/${encodeURIComponent(username)}`)),leagues:async(managerId,season)=>Promise.all((await get<SleeperLeague[]>(`/user/${managerId}/leagues/nfl/${season}`)).map(l=>hydrateSleeperLeague(l,managerId))),league:id=>sleeperLeague(id),draft:async id=>normalizeDraft(await get<SleeperDraft>(`/draft/${id}`),await get<SleeperPick[]>(`/draft/${id}/picks`)),picks:async id=>normalizePicks(await get<SleeperPick[]>(`/draft/${id}/picks`))};

export async function leagueHistory(startId:string,maxSeasons=5,currentUserId?:string){const history:NormalizedLeagueProfile[]=[];let id:string|undefined=startId;while(id&&history.length<maxSeasons+1){try{const league=await sleeperLeague(id,currentUserId);history.push(league);id=league.previousLeagueId}catch(error){if(!history.length)throw error;break}}return history;}
