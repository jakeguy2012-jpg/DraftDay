...
export interface NormalizedKeeper { playerExternalId:string; playerName?:string; teamExternalId:string; round?:number; sourcePickId?:string; originalPickMetadata?:Record<string,string> }
export interface NormalizedDraftPick { externalId:string; pick:number; round:number; slot:number; playerExternalId:string; playerName?:string; teamExternalId?:string; isKeeper:boolean; selectedAt?:Date; metadata?:Record<string,string> }
...
export interface NormalizedADP {sourceId?:string;name:string;team?:string;position:Position;mean:number;deviation:number;format:string;leagueSize:number;season:number;capturedAt:Date}
export interface NormalizedProjection {sourceId?:string;name:string;team?:string;position:Position;stats:StatLine;fantasyPoints?:number;rank?:number;positionRank?:number;capturedAt:Date}
