...
For Vercel, configure a serverless PostgreSQL `DATABASE_URL` (for example Neon), run `npm run db:deploy` as a controlled release step, and use `npm run vercel-build`. Docker is optional and is not used by the Vercel runtime.
...
- `DEMO_MODE=false`
- `DATABASE_URL=` — pooled serverless PostgreSQL URL (Neon is supported)
